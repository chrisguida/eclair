...
class PaymentOnionSpec extends AnyFunSuite {
  ...
  test("encode/decode channel relay per-hop payload") {
    ...
    for ((expected, bin) <- testCases) {
      ...
      assert(decoded === ChannelRelayPayload(expected))
      ...
      val encoded = channelRelayPerHopPayloadCodec.encode(ChannelRelayPayload(expected)).require.bytes
      assert(encoded === bin)
    }
  }
  ...
  test("encode/decode final per-hop payload") {
    ...
    for ((expected, bin) <- testCases) {
      ...
      assert(decoded === FinalPayload(expected))
      assert(decoded.amount === 561.msat)
      assert(decoded.expiry === CltvExpiry(42))

      val encoded = finalPerHopPayloadCodec.encode(FinalPayload(expected)).require.bytes
      assert(encoded === bin)
    }
  }
  ...
  test("encode/decode final per-hop payload with custom user records") {
    ...
    val encoded = finalPerHopPayloadCodec.encode(FinalPayload(tlvs)).require.bytes
    assert(encoded === bin)
    assert(finalPerHopPayloadCodec.decode(bin.bits).require.value == FinalPayload(tlvs))
  }
