...
class RouteCalculationSpec extends AnyFunSuite with ParallelTestExecution {
  ...
  test("find multipart route to neighbor (restricted htlc_minimum_msat)") {
    ...
    {
      val Success(route1 :: route2 :: Nil) = findMultiPartRoute(g, a, b, 10000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
      assert(route2Ids(route1) == 1 :: Nil)
      assert(route2Ids(route2) == 2 :: Nil)
    }
    {
      val Success(routes) = findMultiPartRoute(g, a, b, 10000 msat, 1 msat, routeParams = DEFAULT_ROUTE_PARAMS.copy(randomize = true), currentBlockHeight = BlockHeight(400000))
