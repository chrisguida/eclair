...
class RouteCalculationSpec extends AnyFunSuite with ParallelTestExecution {
  ...
  test("calculate route and return metadata") {
    ...
    val Success(route :: Nil) = findRoute(g, a, e, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, numRoutes = 1, routeParams = DEFAULT_ROUTE_PARAMS, currentBlockHeight = BlockHeight(400000))
    assert(route.hops === ChannelHop.fromChannelUpdate(a, b, uab) :: ChannelHop.fromChannelUpdate(b, c, ubc) :: ChannelHop.fromChannelUpdate(c, d, ucd) :: ChannelHop.fromChannelUpdate(d, e, ude) :: Nil)
  }
...
object RouteCalculationSpec {
  ...
  def makeChannel(shortChannelId: Long, nodeIdA: PublicKey, nodeIdB: PublicKey): ChannelAnnouncement = {
    ...
    ChannelAnnouncement(DUMMY_SIG, DUMMY_SIG, DUMMY_SIG, DUMMY_SIG, Features.empty, Block.RegtestGenesisBlock.hash, ShortChannelId(shortChannelId).toReal, nodeId1, nodeId2, randomKey().publicKey, randomKey().publicKey)
  }
