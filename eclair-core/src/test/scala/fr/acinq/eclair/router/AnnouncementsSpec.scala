...
class AnnouncementsSpec extends AnyFunSuite {
  ...
  test("create valid signed channel announcement") {
    ...
    val witness = Announcements.generateChannelAnnouncementWitness(Block.RegtestGenesisBlock.hash, ShortChannelId(42L).toReal, node_a.publicKey, node_b.publicKey, bitcoin_a.publicKey, bitcoin_b.publicKey, Features.empty)
    ...
    val ann = makeChannelAnnouncement(Block.RegtestGenesisBlock.hash, ShortChannelId(42L).toReal, node_a.publicKey, node_b.publicKey, bitcoin_a.publicKey, bitcoin_b.publicKey, node_a_sig, node_b_sig, bitcoin_a_sig, bitcoin_b_sig)
