...
class RouterSpec extends BaseRouterSpec {
  ...
  test("properly announce valid new channels and ignore invalid ones") { fixture =>
...

    {
      ...
      val chan_ac = channelAnnouncement(ShortChannelId(BlockHeight(420000), 5, 0).toReal, priv_a, priv_c, priv_funding_a, priv_funding_c)
...

    {
      ...
      val chan_uc = channelAnnouncement(ShortChannelId(BlockHeight(420000), 100, 0).toReal, priv_u, priv_c, priv_funding_u, priv_funding_c)
  ...
  test("route not found (private channel disabled)") { fixture =>
    ...
    sender.send(router, LocalChannelUpdate(sender.ref, null, Some(scid_ag_private), alias_ag_private, g, None, channelUpdate_ag1, CommitmentsSpec.makeCommitments(10000 msat, 15000 msat, a, g, announceChannel = false)))
    sender.send(router, RouteRequest(a, h, DEFAULT_AMOUNT_MSAT, DEFAULT_MAX_FEE, routeParams = DEFAULT_ROUTE_PARAMS))
    sender.expectMsg(Failure(RouteNotFound))
  }
  ...
  test("route not found (balance too low)") { fixture =>
    ...
    sender.send(router, LocalChannelUpdate(sender.ref, null, Some(scid_ab), scid_ab, b, Some(chan_ab), update_ab, commitments1))
