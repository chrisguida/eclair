...
class PaymentIntegrationSpec extends IntegrationSpec {
  ...
  test("send an HTLC A->D with an invalid expiry delta for B") {
    ...
    nodes("B").system.eventStream.publish(LocalChannelUpdate(system.deadLetters, commitmentBC.channelId, Some(shortIdBC), shortIdBC, commitmentBC.remoteParams.nodeId, None, channelUpdateBC, commitmentBC))
...
//    sender.send(nodes("B").relayer, Relayer.GetOutgoingChannels())
//    val og = sender.expectMsgType[Relayer.OutgoingChannels]
//    val channelUpdate_ba = og.channels.filter(c => c.nextNodeId == nodes("A").nodeParams.nodeId).head.channelUpdate
