    ...
    case Event(open: OpenDualFundedChannel, d: DATA_WAIT_FOR_OPEN_DUAL_FUNDED_CHANNEL) =>
      ...
      Helpers.validateParamsNonInitiator(nodeParams, d.init.channelType, open, remoteNodeId, localParams.initFeatures, remoteInit.features) match {
        ...
        case Right((channelFeatures, remoteShutdownScript)) =>
          ...
          goto(WAIT_FOR_DUAL_FUNDING_CREATED) using DATA_WAIT_FOR_DUAL_FUNDING_CREATED(channelId, txBuilder) sending accept
      }
    ...
    case Event(accept: AcceptDualFundedChannel, d: DATA_WAIT_FOR_ACCEPT_DUAL_FUNDED_CHANNEL) =>
      ...
      Helpers.validateParamsInitiator(nodeParams, d.init.channelType, localParams.initFeatures, remoteInit.features, d.lastSent, accept) match {
        ...
        case Right((channelFeatures, remoteShutdownScript)) =>
          ...
          txBuilder ! InteractiveTxBuilder.Start(self, Nil)
          goto(WAIT_FOR_DUAL_FUNDING_CREATED) using DATA_WAIT_FOR_DUAL_FUNDING_CREATED(channelId, txBuilder)
      }
    ...
    case Event(msg: InteractiveTxBuilder.Response, d: DATA_WAIT_FOR_DUAL_FUNDING_CREATED) => msg match {
      ...
      case InteractiveTxBuilder.Succeeded(fundingParams, fundingTx, commitments) =>
        ...
        fundingTx match {
          ...
          case fundingTx: FullySignedSharedTransaction => goto(WAIT_FOR_DUAL_FUNDING_CONFIRMED) using nextData storing() sending fundingTx.localSigs calling publishFundingTx(nextData)
        }
    ...
    case Event(msg: InteractiveTxBuilder.Response, d: DATA_WAIT_FOR_DUAL_FUNDING_CONFIRMED) => msg match {
      ...
      case InteractiveTxBuilder.Succeeded(fundingParams, fundingTx, commitments) =>
        ...
        fundingTx match {
          ...
          case fundingTx: FullySignedSharedTransaction => stay() using nextData storing() sending fundingTx.localSigs calling publishFundingTx(nextData)
        }
      case f: InteractiveTxBuilder.Failed =>
          ...
          Try(Transaction.correctlySpends(commitments.fullySignedLocalCommitTx(keyManager).tx, Seq(confirmedTx), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)) match {
            case Success(_) =>
              ...
              val toSend = d.rbfAttempt match {
                case Some(Right(txBuilder)) =>
                  txBuilder ! InteractiveTxBuilder.Abort
                  Seq(TxAbort(d.channelId, InvalidRbfTxConfirmed(d.channelId).getMessage), fundingLocked)
                case Some(Left(cmd)) =>
              ...
              goto(WAIT_FOR_DUAL_FUNDING_LOCKED) using DATA_WAIT_FOR_DUAL_FUNDING_LOCKED(commitments, shortChannelId, otherFundingTxs, fundingLocked) storing() sending toSend
            case Failure(t) =>
