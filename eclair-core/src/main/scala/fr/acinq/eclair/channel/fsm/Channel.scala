    ...
    case state -> nextState =>
      ...
      emitEvent_opt.foreach {
        case EmitLocalChannelUpdate(reason, d, sendToPeer) =>
          ...
          context.system.eventStream.publish(LocalChannelUpdate(self, d.channelId, localAlias = d.localAlias, d.commitments.remoteParams.nodeId, d.channelAnnouncement, d.channelUpdate, d.commitments))
          if (sendToPeer) {
