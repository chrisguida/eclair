...
case class LocalCommitPublished(commitTx: Transaction,
  ...
  def isDone: Boolean = {
    ...
    val isMainOutputConfirmed = claimMainDelayedOutputTx match {
    ...
    val unconfirmedHtlcDelayedTxs = claimHtlcDelayedTxs.flatMap(_.toOption).map(_.input.outPoint) // TODO replace .flatMap(_.toOption) by matching all cases like above
...
case class RemoteCommitPublished(commitTx: Transaction,
  ...
  def isDone: Boolean = {
    ...
    val isMainOutputConfirmed = claimMainOutputTx_opt match {
      case Some(claimMainOutputTx) => claimMainOutputTx match {
        case TxGenerationResult.Success(tx) => irrevocablySpent.contains(tx.input.outPoint)
        case TxGenerationResult.OutputNotFound => true
        case TxGenerationResult.AmountBelowDustLimit => true
        case _: TxGenerationResult.Failure => false // we may have a bug preventing us from spending this output for now
    ...
    val allHtlcsSpent = claimHtlcTxs.forall {
      ...
      case (outPoint, RemoteCommitPublished.HtlcOutputStatus.PendingDownstreamSettlement) => irrevocablySpent.contains(outPoint)
...
case class RevokedCommitPublished(commitTx: Transaction,
  ...
  def isDone: Boolean = {
    ...
    val unspentCommitTxOutputs = { // TODO: fix this
      val commitOutputsSpendableByUs = (claimMainOutputTx_opt.flatMap(_.toOption).toSeq ++ mainPenaltyTx.toOption.toSeq ++ htlcPenaltyTxs.flatMap(_.toOption)).map(_.input.outPoint)
      commitOutputsSpendableByUs.toSet -- irrevocablySpent.keys
    }
    // are all outputs from htlc txs spent?
    val unconfirmedHtlcDelayedTxs = claimHtlcDelayedPenaltyTxs.flatMap(_.toOption).map(_.input.outPoint)
      // only the txs which parents are already confirmed may get confirmed (note that this eliminates outputs that have been double-spent by a competing tx)
      .filter(input => confirmedTxs.contains(input.txid))
