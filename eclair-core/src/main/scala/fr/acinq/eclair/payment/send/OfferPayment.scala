  ...
  def sendInvoiceRequest(nodeParams: NodeParams,
    ...
    offer.contact match {
            case Left(blindedRoute) =>
              ...
              waitForInvoice(nodeParams, postman, paymentInitiator, offer, Map(pathId -> offer.nodeId.nodeId1), request, payerKey, replyTo, remainingAttempts - 1, sendPaymentConfig)
            case Right((nodeId1, nodeId2)) =>
              ...
              // same person).
              val (pathId1, nextNodeId1, message1) = buildRequest(nodeParams, request, Recipient(nodeId1, None, None))
              val (pathId2, nextNodeId2, message2) = buildRequest(nodeParams, request, Recipient(nodeId2, None, None))
              ...
              waitForInvoice(nodeParams, postman, paymentInitiator, offer, Map(pathId1 -> nodeId1, pathId2 -> nodeId2),request, payerKey, replyTo, remainingAttempts - 1, sendPaymentConfig)
