...
import fr.acinq.eclair.{BlockHeight, FeatureSupport, Features, ShortChannelId}
  ...
  private[version3] object Codecs {
    ...
    val DATA_NORMAL_Codec: Codec[DATA_NORMAL] = (
      ("commitments" | commitmentsCodec) ::
        ...
        ("channelAnnouncement" | optional(bool8, lengthDelimited(channelAnnouncementCodec))) ::
        ("channelUpdate" | lengthDelimited(channelUpdateCodec)) ::
        ("localAlias" | discriminated[ShortChannelId].by(uint16).typecase(1, shortchannelid)) :: // forward-compatible with listOfN(uint16, shortchannelid) in case we want to store a list of local aliases later
