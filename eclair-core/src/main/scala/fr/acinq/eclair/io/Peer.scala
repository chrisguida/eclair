    ...
    dropStaleMessages {
      ...
      case Event(c: Peer.OpenChannel, d: ConnectedData) =>
        if (c.fundingSatoshis >= Channel.MAX_FUNDING && !d.localFeatures.hasFeature(Wumbo)) {
          ...
          val (selfRef, origin) = (self, sender())
          ...
          createLocalParams(nodeParams, d.localFeatures, channelType, isInitiator = true, c.fundingSatoshis).andThen {
            ...
            case Failure(t) => origin ! Status.Failure(new RuntimeException(s"channel creation failed: reason=${t.getMessage}"))
          }
          stay()
        }
      ...
      case Event(SpawnChannelNonInitiator(msg, channelConfig, channelType, localParams), d: ConnectedData) =>
